...
export default function LessonModal({ students, defaultDate, existingLesson, onClose, onSaved }) {
  ...
  if (form.subject) {
    msgParts.push(`📖 Konu: *${form.subject}*`);
  }

  if (form.location) {
    msgParts.push(`📌 Konum: ${form.location}`);
  }

  if (meetingLink) {
    msgParts.push(`🔗 Ders Linki: ${meetingLink}`);
  }

  msgParts.push(
    '',
    '✨ Öğrencimizin gelişimini desteklemek ve verimli bir ders geçirmek için sabırsızlanıyoruz.',
    '',
    'Şimdiden güzel ve verimli bir ders olmasını dileriz. 🎓📖'
  );
