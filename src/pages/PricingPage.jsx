    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'Inter', sans-serif", color: '#1e293b' }}>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '60px 5%' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <span style={{ display: 'inline-block', background: 'linear-gradient(135deg,#f5f3ff,#ede9fe)', color: '#6d28d9', fontWeight: 800, fontSize: '.75rem', textTransform: 'uppercase', letterSpacing: 2, borderRadius: 25, padding: '.4rem 1.1rem', marginBottom: '1rem' }}>Fiyatlandırma</span>
          <h1 style={{ fontSize: 'clamp(1.8rem,4vw,2.8rem)', fontWeight: 900, color: '#111827', marginBottom: '.75rem', letterSpacing: '-.8px' }}>Şeffaf & Esnek Fiyatlandırma</h1>
          <p style={{ color: '#6b7280', fontSize: '1.05rem', maxWidth: 520, margin: '0 auto', lineHeight: 1.7 }}>
            Sadece aktif öğrenci sayınız kadar ödeyin. Pasif öğrenciler ücretsiz. İlk 30 gün tamamen ücretsiz.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px,1fr))', gap: 28, alignItems: 'start' }}>
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: '2rem', border: '1.5px solid #e5e7eb', boxShadow: '0 2px 12px rgba(0,0,0,.04)' }}>
            <h3 style={{ fontWeight: 800, fontSize: '1.1rem', color: '#111827', marginBottom: '.25rem' }}>Ücretsiz</h3>
            <p style={{ fontSize: '.82rem', color: '#9ca3af', marginBottom: '1rem' }}>Başlamak için ideal</p>
            <div style={{ marginBottom: '1.5rem' }}>
              <span style={{ fontSize: '2.8rem', fontWeight: 900, color: '#111827' }}>₺0</span>
              <span style={{ fontSize: '.85rem', color: '#9ca3af', marginLeft: '.4rem' }}>/ sonsuza kadar</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '.6rem', marginBottom: '1.75rem' }}>
              {['3 aktif öğrenciye kadar', 'Ders takvimi', 'Temel finans takibi', 'Veli paneli'].map(f =>
          ...
          <div style={{ background: 'white', borderRadius: 22, padding: '2rem', border: '1.5px solid #ddd6fe', boxShadow: '0 16px 48px rgba(79,70,229,.14)', position: 'relative' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ fontWeight: 800, fontSize: '1.1rem', color: '#111827', marginBottom: '.6rem' }}>Pro Paket Hesaplayıcı</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginBottom: '1rem' }}>
              <div style={{ background: '#f8fafc', border: '1.5px solid #e5e7eb', borderRadius: 12, padding: '.85rem', textAlign: 'center' }}>
                <div style={{ color: '#6b7280', fontSize: '.75rem', fontWeight: 700, marginBottom: '.25rem' }}>Net Tutar</div>
                <div style={{ color: '#111827', fontSize: '1.55rem', fontWeight: 900 }}>{totalNet.toLocaleString('tr-TR')}₺</div>
              </div>
              <div style={{ background: '#f8fafc', border: '1.5px solid #e5e7eb', borderRadius: 12, padding: '.85rem', textAlign: 'center' }}>
                <div style={{ color: '#6b7280', fontSize: '.75rem', fontWeight: 700, marginBottom: '.25rem' }}>KDV (%20)</div>
            ...
            <div style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', borderRadius: 12, padding: '.85rem', textAlign: 'center', marginBottom: '1rem' }}>
              <div style={{ color: 'rgba(255,255,255,.8)', fontSize: '.75rem', fontWeight: 700, marginBottom: '.2rem' }}>Aylık Toplam (KDV Dahil)</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.5rem', marginBottom: '1.25rem' }}>
              {['30 gün ücretsiz deneme', 'Tüm özellikler', 'Pasif öğrenciler ücretsiz', 'İstediğiniz zaman iptal', 'AI ders raporu', 'Gelişmiş finans', 'Veli paneli', 'Öncelikli destek'].map(f =>
            ...
            <p style={{ margin: '.6rem 0 0', textAlign: 'center', color: '#9ca3af', fontSize: '.8rem' }}>Kredi kartı gerekmez</p>
          </div>
        </div>

        {/* SSS */}
        <div style={{ marginTop: 64, background: 'white', borderRadius: 20, padding: '2rem', border: '1.5px solid #e5e7eb' }}>
          <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: '#111827', marginBottom: '1.5rem' }}>Sıkça Sorulan Sorular</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px,1fr))', gap: '1.5rem' }}>
            {[
              { q: 'Pasif öğrenciler ücretlendirilir mi?', a: 'Hayır. Sadece "aktif" statüsündeki öğrenciler için ücret ödersiniz.' },
              { q: 'Ay ortasında öğrenci ekleyebilir miyim?', a: 'Evet. Ay ortasında eklenen öğrenciler için kalan günler oranında pro-rata ücretlendirme yapılır.' },
              { q: 'İstediğim zaman iptal edebilir miyim?', a: 'Evet, herhangi bir zamanda aboneliğinizi iptal edebilirsiniz. Kalan süre aktif kalır.' },
              { q: 'KDV nasıl hesaplanıyor?', a: 'Gösterilen fiyatlara %20 KDV eklenmektedir. Ödeme sayfasında toplam tutar KDV dahil gösterilir.' },
            ].map(({ q, a }) =>
              <div key={q}>
