...
// ─────────────────────────────────────────────
// GRADES — StudentDetailModal.jsx ile birebir aynı
// ─────────────────────────────────────────────
const GRADES = [
...
// ─────────────────────────────────────────────
// FALLBACK MOCK DATA
// Alan adları: gerçek Student entity ile uyumlu
//   feePerLesson   → StudentCard.jsx + StudentDetailModal.jsx
//   grade          → StudentDetailModal.jsx (GRADES dizisinden)
//   weeklyLessons  → StudentCard.jsx
// ─────────────────────────────────────────────
const FALLBACK_STUDENTS = [
  {
    id: "mock-1", name: "Elif Kaya", status: "active",
    feePerLesson: 450, grade: "11. Sınıf", subject: "Matematik",
    weeklyLessons: 2, lessonDuration: 60,
    city: "İstanbul", district: "Kadıköy",
    lastRaiseDate: "2024-09-01",
    lessonType: "Yüz Yüze", schoolType: "Devlet",
    parentPhone: "5321234567",
  },
  {
    id: "mock-2", name: "Ahmet Demir", status: "active",
    feePerLesson: 350, grade: "Ortaokul (5-8)", subject: "Fen Bilimleri",
    weeklyLessons: 3, lessonDuration: 60,
    city: "Ankara", district: "Çankaya",
    lastRaiseDate: "2024-06-15",
    lessonType: "Online", schoolType: "Devlet",
    parentPhone: "5339876543",
  },
  {
    id: "mock-3", name: "Zeynep Arslan", status: "active",
    feePerLesson: 650, grade: "12. Sınıf", subject: "İngilizce",
    weeklyLessons: 2, lessonDuration: 90,
    city: "İstanbul", district: "Beşiktaş",
    lastRaiseDate: "2025-01-10",
    lessonType: "Yüz Yüze", schoolType: "Kolej-Özel",
    parentPhone: "5305556677",
  },
  {
    id: "mock-4", name: "Can Yılmaz", status: "active",
    feePerLesson: 400, grade: "İlkokul (1-4)", subject: "Türkçe",
    weeklyLessons: 1, lessonDuration: 45,
    city: "İzmir", district: "Bornova",
    lastRaiseDate: "2024-03-20",
    lessonType: "Yüz Yüze", schoolType: "Devlet",
    parentPhone: null,
  },
  {
    id: "mock-5", name: "Selin Çelik", status: "active",
    feePerLesson: 550, grade: "10. Sınıf", subject: "Fizik",
    weeklyLessons: 2, lessonDuration: 60,
    city: "Bursa", district: null,
    lastRaiseDate: "2024-11-05",
    lessonType: "Online", schoolType: "Kolej-Özel",
    parentPhone: "5441112233",
  },
  {
    id: "mock-6", name: "Mert Özkan", status: "active",
    feePerLesson: 300, grade: "Ortaokul (5-8)", subject: "Matematik",
    weeklyLessons: 2, lessonDuration: 60,
    city: "Konya", district: null,
    lastRaiseDate: "2023-09-01",
    lessonType: "Online", schoolType: "Devlet",
    parentPhone: "5367778899",
  },
];

// ─────────────────────────────────────────────
// normalizeStudent
// base44 Student entity → Page1 iç modeli
// Kaynak: StudentCard.jsx + StudentDetailModal.jsx
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// ŞEHİR ÇARPAN TABLOSU
// ─────────────────────────────────────────────
const CITY_MULTIPLIER = {
  İstanbul: 1.20, Ankara: 1.12, İzmir: 1.10,
  Kocaeli: 1.08, Bursa: 1.07, Antalya: 1.06,
  Eskişehir: 1.05, Muğla: 1.05, Tekirdağ: 1.04,
  Yalova: 1.04, Balıkesir: 1.03, Sakarya: 1.03,
  Gaziantep: 1.02, Kayseri: 1.01, Mersin: 1.01,
  Denizli: 1.01, Samsun: 1.00, Trabzon: 1.00,
  Manisa: 1.00, Konya: 0.99, Adana: 0.98,
  Hatay: 0.97, Malatya: 0.97, Kahramanmaraş: 0.96,
  Diyarbakır: 0.95, Şanlıurfa: 0.94, Elazığ: 0.94,
  Erzurum: 0.94, Van: 0.93, Mardin: 0.93,
  Siirt: 0.92, Batman: 0.92, Şırnak: 0.91,
  Hakkari: 0.91, Iğdır: 0.91, Ağrı: 0.91,
};
...
const S = {
  page: {
    ...
    color: "#1e1b4b", padding: "0 0 60px 0",
  },
  header: {
    background: "linear-gradient(135deg, #4f46e5 0%, #3730a3 50%, #1e40af 100%)",
  ...
  headerBadge: {
    ...
    padding: "4px 12px", fontSize: 12, color: "#c7d2fe",
    fontWeight: 500, marginBottom: 14, letterSpacing: 0.5,
  },
  ...
  sectionLabel: {
    fontSize: 11, fontWeight: 700, letterSpacing: 1.2, color: "#6366f1",
  ...
  card: {
    ...
    border: "1px solid rgba(99,102,241,0.08)", padding: "20px 24px", transition: "box-shadow 0.2s",
  },
  searchInput: {
    width: "100%", padding: "10px 14px 10px 36px", borderRadius: 10,
    border: "1.5px solid #e0e7ff", background: "#f8f7ff",
    fontSize: 14, color: "#1e1b4b", outline: "none", boxSizing: "border-box",
  },
  studentGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 12, marginTop: 14 },
  studentCard: (sel) => ({
    borderRadius: 12, border: sel ? "2px solid #6366f1" : "1.5px solid #e0e7ff",
    background: sel ? "linear-gradient(135deg, #eef2ff, #f5f3ff)" : "#fafafe",
    padding: "14px 16px", cursor: "pointer", transition: "all 0.2s",
    transform: sel ? "translateY(-1px)" : "none",
    boxShadow: sel ? "0 4px 16px rgba(99,102,241,0.15)" : "none", position: "relative",
  }),
  badge: (c) => ({
    display: "inline-block",
    background: c === "purple" ? "#ede9fe" : c === "blue" ? "#dbeafe" : "#dcfce7",
    color:      c === "purple" ? "#6d28d9" : c === "blue" ? "#1d4ed8" : "#16a34a",
    borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600,
  }),
  summaryCard: {
    background: "linear-gradient(135deg, #4f46e5, #3730a3)", borderRadius: 16,
    padding: "22px 24px", color: "#fff", boxShadow: "0 8px 32px rgba(79,70,229,0.25)", marginTop: 20,
  },
  ...
  input: {
    ...
    border: "1.5px solid #e0e7ff", background: "#f8f7ff",
    fontSize: 14, color: "#1e1b4b", outline: "none", boxSizing: "border-box", fontFamily: "inherit",
  },
  select: {
    width: "100%", padding: "10px 12px", borderRadius: 10,
    border: "1.5px solid #e0e7ff", background: "#f8f7ff",
  ...
  analyzeBtn: (loading) => ({
    ...
    background: loading ? "linear-gradient(135deg, #a5b4fc, #818cf8)" : "linear-gradient(135deg, #6366f1, #4f46e5)",
  ...
  resultCard: (type) => {
    ...
    return { borderRadius:16, border:`2px solid ${m[type].border}`, background:m[type].bg, padding:"22px 20px", position:"relative",
      boxShadow: type==="balanced" ? "0 8px 32px rgba(79,70,229,0.25)" : "0 2px 8px rgba(0,0,0,0.04)",
      transform: type==="balanced" ? "scale(1.02)" : "scale(1)", transition:"all 0.3s" };
  },
  ...
  emptyState: { textAlign:"center", padding:"48px 24px", color:"#a5b4fc" },
...
export default function Page1() {
  ...
  // ── STUDENT SELECTION: base44 (TeacherStudents.jsx mantığı) ──
  useEffect(() => {
    let cancelled = false;
  ...
  const handleSelectStudent = useCallback((student) => {
    ...
    setForm((prev) => ({
      ...
      level:           student.schoolLevel     || "",   // grade → schoolLevel
      lessonType:      student.lessonType      || "",
      schoolType:      student.schoolType      || "",
      currentPrice:    String(student.currentHourlyPrice || ""), // feePerLesson → currentHourlyPrice
      lastRaiseDate:   student.lastRaiseDate   || "",
      district:        student.district        || "",
      weeklyFrequency: String(student.weeklyFrequency || ""),    // weeklyLessons → weeklyFrequency
    ...
    <div style={S.page}>
      ...
      <div style={S.container}>
        ...
        <div style={S.section}>
          ...
          <div style={S.card}>
            {studentsError && (
              <div style={{ marginBottom:12, padding:"8px 14px", background:"#fef3c7", borderRadius:8, fontSize:12, color:"#92400e", border:"1px solid #fde68a" }}>
                ⚠️ {studentsError}
              </div>
            )}
            ...
            <div style={S.card}>
              ...
              <div style={S.grid2}>
                <div>
                  ...
                  {/* Seviye — GRADES (StudentDetailModal.jsx) */}
                  <div style={S.formGroup}>
                ...
                <div>
                  ...
                  {/* Ücret — feePerLesson'dan otomatik dolar */}
                  <div style={S.formGroup}>
...
ALAN EŞLEMESİ — normalizeStudent() fonksiyonu:
  Student entity   →  Page1 iç model          Kaynak dosya
  ──────────────────────────────────────────────────────────
  feePerLesson     →  currentHourlyPrice       StudentCard.jsx + StudentDetailModal.jsx
  grade            →  schoolLevel              StudentDetailModal.jsx (GRADES dizisi)
  weeklyLessons    →  weeklyFrequency          StudentCard.jsx
  lessonDuration   →  (summary'de gösterilir)  StudentDetailModal.jsx

MOCK DATA (FALLBACK_STUDENTS):
  • base44 API erişimi olmadığında veya sıfır kayıt dönünce devreye girer
  • Canlı ortamda otomatik devre dışı kalır

STUDENT ENTITY'YE EKLENMESİ GEREKEN ALANLAR:
  • lessonType    (Online / Yüz Yüze)   → şu an yoksa form boş gelir, öğretmen manuel girer
  • schoolType    (Devlet / Kolej-Özel) → aynı şekilde
  • lastRaiseDate (son zam tarihi)      → Payment geçmişinden de türetilebilir
  • city / district                     → şu an yoksa form boş gelir

ŞEHİR LİSTESİ:
  • ALL_CITIES: Türkiye'nin tüm 81 ili, alfabetik sırayla
  • CITY_MULTIPLIER: 36 büyükşehir/bölge için özel çarpan; geri kalanlar 1.0

SEVİYE LİSTESİ:
  • GRADES: StudentDetailModal.jsx'teki dizi ile birebir aynı
─────────────────────────────────────────────────────────────────
*/